...
describe("PositionValue", async () => {
  ...
  const positionValueCompleteFixture: Fixture<{
    ...
    const positionValue =
      (await positionValueFactory.deploy()) as PositionValueTest;

    for (const token of tokens) {
      await token.approve(nft.address, constants.MaxUint256);
      await token
        .connect(wallets[0])
        .approve(nft.address, constants.MaxUint256);
  ...
  beforeEach(async () => {
    ...
    pool = new ethers.Contract(poolAddress, IElixirPoolABI, wallets[0]);
  });
  ...
  describe("#total", () => {
    ...
    it("gas", async () => {
      await snapshotGasCost(
        positionValue.totalGas(nft.address, 1, sqrtRatioX96)
      );
    });
  });
  ...
  describe("#principal", () => {
    ...
    it("gas", async () => {
      ...
      await snapshotGasCost(
        positionValue.principalGas(nft.address, 1, sqrtRatioX96)
      );
    });
  });
