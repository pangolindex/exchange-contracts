...
describe("SwapRouter gas tests", function () {
  ...
  async function exactOutputSingle(
    ...
    if (inputIsWETH9)
      data.push(
        router.interface.encodeFunctionData("unwrapWETH9", [0, trader.address])
      );
    if (outputIsWETH9)
      data.push(
        router.interface.encodeFunctionData("unwrapWETH9", [
  ...
  beforeEach("ensure feeGrowthGlobals are >0", async () => {
    ...
    expect(slots).to.deep.eq([
      [
        "340290874192793283295456993856614",
        "340290874192793283295456993856614",
      ],
      [
        "340290874192793283295456993856614",
  ...
  beforeEach("ensure ticks are 0 before", async () => {
    const slots = await Promise.all(
      pools.map((pool) => pool.slot0().then(({ tick }) => tick))
    );
  ...
  describe("#exactInput", () => {
    it("0 -> 1", async () => {
      await snapshotGasCost(
        exactInput(tokens.slice(0, 2).map((token) => token.address))
      );
    });
    ...
    it("0 -> 1 minimal", async () => {
      ...
      await tokens[0]
        .connect(trader)
        .approve(callee.address, constants.MaxUint256);
      await snapshotGasCost(
        callee
          .connect(trader)
          .swapExact0For1(pools[0].address, 2, trader.address, "4295128740")
      );
    });
  ...
  describe("#exactInputSingle", () => {
    it("0 -> 1", async () => {
      await snapshotGasCost(
        exactInputSingle(tokens[0].address, tokens[1].address)
      );
    });
  ...
  describe("#exactOutput", () => {
    it("0 -> 1", async () => {
      await snapshotGasCost(
        exactOutput(tokens.slice(0, 2).map((token) => token.address))
      );
    });
  ...
  describe("#exactOutputSingle", () => {
    it("0 -> 1", async () => {
      await snapshotGasCost(
        exactOutputSingle(tokens[0].address, tokens[1].address)
      );
    });

    it("WETH9 -> 0", async () => {
      await snapshotGasCost(
        exactOutputSingle(weth9.address, tokens[0].address)
      );
    });

    it("0 -> WETH9", async () => {
      await snapshotGasCost(
        exactOutputSingle(tokens[0].address, weth9.address)
      );
    });
  });
});
