...
describe("PoolTicksCounter", () => {
  ...
  TICK_SPACINGS.forEach((TICK_SPACING) => {
    ...
    before(async () => {
      ...
      PoolTicksCounter =
        (await poolTicksHelperFactory.deploy()) as PoolTicksCounterTest;
    ...
    describe(`[Tick Spacing: ${TICK_SPACING}]: tick after is bigger`, async () => {
      ...
      it("counts ticks when before and after are initialized on same page", async () => {
        ...
        const startingTickInit =
          await PoolTicksCounter.countInitializedTicksCrossed(
            pool.address,
            bitIdxToTick(2),
            bitIdxToTick(255)
          );
        expect(startingTickInit).to.be.eq(5);
        const endingTickInit =
          await PoolTicksCounter.countInitializedTicksCrossed(
            pool.address,
            bitIdxToTick(0),
            bitIdxToTick(3)
