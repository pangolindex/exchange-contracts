...
describe("SwapRouter", function () {
  ...
  beforeEach("load fixture", async () => {
    ({ router, weth9, factory, tokens, nft } = await loadFixture(
      swapRouterFixture
    ));
  ...
  it("bytecode size", async () => {
    expect(
      ((await router.provider.getCode(router.address)).length - 2) / 2
    ).to.matchSnapshot();
  });
      ...
      describe("ETH input", () => {
        describe("WETH9", () => {
          ...
          it("WETH9 -> 0 -> 1", async () => {
            ...
            await expect(
              exactInput(
                [weth9.address, tokens[0].address, tokens[1].address],
                5
              )
            )
              .to.emit(weth9, "Deposit")
      ...
      describe("ETH output", () => {
        describe("WETH9", () => {
          ...
          it("0 -> 1 -> WETH9", async () => {
            ...
            await expect(
              exactInput(
                [tokens[0].address, tokens[1].address, weth9.address],
                5
              )
            )
              .to.emit(weth9, "Withdrawal")
