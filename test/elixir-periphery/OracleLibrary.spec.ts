...
describe("OracleLibrary", () => {
  ...
  const oracleTestFixture = async () => {
    ...
    const oracleFactory = await ethers.getContractFactory(
      "contracts/elixir-periphery/test/OracleTest.sol:OracleTest"
    );
  ...
  describe("#consult", () => {
    ...
    function calculateHarmonicAvgLiq(
      ...
      const [secondsPerLiq0, secondsPerLiq1] = secondsPerLiqCumulatives.map(
        BigNumber.from
      );
  ...
  describe("#getOldestObservationSecondsAgo", () => {
    ...
    const runOldestObservationsTest = async (
      ...
      var result = await oracle.getOldestObservationSecondsAgo(
        mockObservations.address
      );
    ...
    before("create mockObservationsFactory", async () => {
      mockObservationsFactory = await ethers.getContractFactory(
        "MockObservations"
      );
    });
  ...
  describe("#getBlockStartingTickAndLiquidity", () => {
    ...
    it("returns the tick and liquidity in storage if the latest observation was in a previous block", async () => {
      ...
      var result = await oracle.getBlockStartingTickAndLiquidity(
        mockObservations.address
      );
    ...
    it("calculates the prior tick and liquidity from the prior observations", async () => {
      ...
      var actualStartingTick =
        (tickCumulatives[0] - tickCumulatives[2]) /
        (blockTimestamps[0] - blockTimestamps[2]);
  ...
  describe("#getWeightedArithmeticMeanTick", () => {
    it("single observation returns average tick", async () => {
      ...
      const oracleTick = await oracle.getWeightedArithmeticMeanTick([
        observation,
      ]);
  ...
  describe("#getChainedPrice", () => {
    ...
    it("fails with discrepant length", async () => {
      ...
      expect(oracle.getChainedPrice(tokenAddresses, ticks)).to.be.revertedWith(
        "DL"
      );
    });
