...
describe("Path", () => {
  ...
  it("js encoding works as expected", async () => {
    let expectedPath =
      ...
      tokenAddresses
        ...
        .join("000bb8");
    expect(encodePath(tokenAddresses.slice(0, 2), fees.slice(0, 1))).to.eq(
      expectedPath
    );
  ...
  describe("#hasMultiplePools / #decodeFirstPool / #skipToken / #getFirstPool", () => {
    ...
    it("works on first pool", async () => {
      ...
      expect(
        await path.decodeFirstPool(await path.getFirstPool(encodedPath))
      ).to.deep.eq(firstPool);
    });
    ...
    it("skips 1 item", async () => {
      ...
      const {
        ...
        tokenB,
        fee: decodedFee,
      } = await path.decodeFirstPool(skipped);
