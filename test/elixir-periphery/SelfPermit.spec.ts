...
describe("SelfPermit", () => {
  ...
  const fixture: Fixture<{
    ...
    const selfPermitTestFactory = await ethers.getContractFactory(
      "SelfPermitTest"
    );
    const selfPermitTest =
      (await selfPermitTestFactory.deploy()) as SelfPermitTest;
  ...
  it("#permit", async () => {
    ...
    expect(await token.allowance(wallet.address, other.address)).to.be.eq(
      value
    );
  });
  ...
  describe("#selfPermit", () => {
    ...
    it("works", async () => {
      ...
      expect(
        await token.allowance(wallet.address, selfPermitTest.address)
      ).to.be.eq(0);
      await selfPermitTest.selfPermit(
        token.address,
        value,
  ...
  describe("#selfPermitAllowed", () => {
    it("works", async () => {
      ...
      await expect(
        selfPermitTest.selfPermitAllowed(
          token.address,
          0,
          constants.MaxUint256,
          v,
