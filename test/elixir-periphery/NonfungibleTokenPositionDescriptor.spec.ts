...
describe("NonfungibleTokenPositionDescriptor", () => {
  ...
  beforeEach("load fixture", async () => {
    ...
    weth9 = tokenFactory.attach(
      await nftPositionDescriptor.WETH9()
    ) as TestERC20;
  });
  ...
  describe("#tokenRatioPriority", () => {
    it("returns -100 for WETH9", async () => {
      expect(
        await nftPositionDescriptor.tokenRatioPriority(weth9.address, 1)
      ).to.eq(-100);
    });

    it("returns 200 for USDC", async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(USDC, 1)).to.eq(
        300
      );
    });
    ...
    it("returns  150 for USDT", async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(USDT, 1)).to.eq(
        200
      );
    });

    it("returns -200 for TBTC", async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(TBTC, 1)).to.eq(
        -200
      );
    });

    it("returns -250 for WBTC", async () => {
      expect(await nftPositionDescriptor.tokenRatioPriority(WBTC, 1)).to.eq(
        -300
      );
    });
    ...
    it("returns 0 for any non-ratioPriority token", async () => {
      expect(
        await nftPositionDescriptor.tokenRatioPriority(tokens[0].address, 1)
      ).to.eq(0);
    });
  });
  ...
  describe("#flipRatio", () => {
    it("returns false if neither token has priority ordering", async () => {
      expect(
        await nftPositionDescriptor.flipRatio(
          tokens[0].address,
          tokens[2].address,
          1
        )
    ...
    it("returns true if both tokens are denominators but token1 has lower priority ordering", async () => {
      expect(
        await nftPositionDescriptor.flipRatio(weth9.address, WBTC, 1)
      ).to.eq(true);
    });
