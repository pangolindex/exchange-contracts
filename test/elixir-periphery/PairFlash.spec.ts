...
describe("PairFlash test", () => {
  ...
  const flashFixture = async () => {
    const { router, tokens, factory, weth9, nft } = await completeFixture(
      wallets,
      provider
    );
  ...
  beforeEach("load fixture", async () => {
    ({ factory, token0, token1, flash, nft, quoter } = await loadFixture(
      flashFixture
    ));
    ...
    it("test correct transfer events", async () => {
      ...
      await expect(flash.initFlash(flashParams))
        ...
        .withArgs(
          flash.address,
          wallet.address,
          expectedAmountOut0.toNumber() - amount0In - fee0
        )
        .to.emit(token1, "Transfer")
        .withArgs(
          flash.address,
          wallet.address,
          expectedAmountOut1.toNumber() - amount1In - fee1
        );
    });
