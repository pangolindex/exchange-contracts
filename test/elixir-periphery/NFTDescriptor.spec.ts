...
describe("NFTDescriptor", () => {
  ...
  const nftDescriptorFixture: Fixture<{
    ...
    const nftDescriptor =
      (await NFTDescriptorFactory.deploy()) as NFTDescriptorTest;
  ...
  describe("#tickToDecimalString", () => {
    ...
    describe("when tickspacing is 10", () => {
      ...
      it("returns MIN on lowest tick", async () => {
        expect(
          await nftDescriptor.tickToDecimalString(
            minTick,
            tickSpacing,
            18,
            18,
      ...
      it("returns the correct decimal string when the tick is in range", async () => {
        expect(
          await nftDescriptor.tickToDecimalString(1, tickSpacing, 18, 18, false)
        ).to.equal("1.0001");
      });
    ...
    describe("when tickspacing is 200", () => {
      ...
      it("returns the correct decimal string when the tick is in range", async () => {
        expect(
          await nftDescriptor.tickToDecimalString(0, tickSpacing, 18, 18, false)
        ).to.equal("1.0000");
      });
    ...
    describe("when token ratio is flipped", () => {
      it("returns the inverse of default ratio for medium sized numbers", async () => {
        ...
        expect(
          await nftDescriptor.tickToDecimalString(10, tickSpacing, 18, 18, true)
        ).to.eq("0.99900");
      });
      ...
      it("returns the inverse of default ratio for large numbers", async () => {
        ...
        ).to.eq("0.00000000000000000000069041");
      });
  ...
  describe("#fixedPointToDecimalString", () => {
    describe("returns the correct string for", () => {
      it("the highest possible price", async () => {
        ...
        expect(
          await nftDescriptor.fixedPointToDecimalString(ratio, 18, 18)
        ).to.eq("338490000000000000000000000000000000000");
      });
      ...
      it("when the decimal is at index 1", async () => {
        ...
        const bla = await nftDescriptor.fixedPointToDecimalString(
          ratio,
          18,
          18
        );
      ...
      it("very small numbers", async () => {
        ...
        expect(
          await nftDescriptor.fixedPointToDecimalString(ratio, 18, 18)
        ).to.eq("0.00000000000000038741");
        ratio = encodePriceSqrt(88498, 10 ** 35);
        expect(
          await nftDescriptor.fixedPointToDecimalString(ratio, 18, 18)
        ).to.eq("0.00000000000000000000000000000088498");
      });
    ...
    describe("when tokens have different decimal precision", () => {
      describe("when baseToken has more precision decimals than quoteToken", () => {
        it("returns the correct string when the decimal difference is even", async () => {
          expect(
            await nftDescriptor.fixedPointToDecimalString(
              encodePriceSqrt(1, 1),
              18,
              16
            )
        ...
        it("returns the correct string when the decimal difference is odd", async () => {
          ...
          expect(
            await nftDescriptor.fixedPointToDecimalString(tenRatio, 18, 17)
          ).to.eq("100.00");
        });
      ...
      describe("when quoteToken has more precision decimals than baseToken", () => {
        ...
        it.skip("returns the correct string when the decimal difference brings ratio below the minimum", async () => {
          ...
          expect(
            await nftDescriptor.fixedPointToDecimalString(lowRatio, 10, 20)
      ...
      it("some fuzz", async () => {
        ...
        let i = 0;
        while (i <= 20) {
          const ratio = BigNumber.from(
            `0x${randomBytes(random(7, 20)).toString("hex")}`
          );
        ...
        for (let i in inputs) {
          ...
          let result = await nftDescriptor.fixedPointToDecimalString(
            ratio,
            decimals0,
            decimals1
          );
  ...
  describe("#svgImage", () => {
    ...
    it("matches the current snapshot", async () => {
      ...
      fs.writeFileSync(
        "./test/elixir-periphery/__snapshots__/NFTDescriptor.svg",
        svg
      );
    });
  ...
  function constructTokenMetadata(
    ...
    return {
      name: `Elixir - ${feeTier} - ${quoteTokenSymbol}/${baseTokenSymbol} - ${prices}`,
      description: `This NFT represents a liquidity position in an Elixir ${quoteTokenSymbol}-${baseTokenSymbol} pool. The owner of this NFT can modify or redeem the position.\n\
