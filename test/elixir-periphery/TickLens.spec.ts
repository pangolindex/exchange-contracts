...
describe("TickLens", () => {
  ...
  describe("#getPopulatedTicksInWord", () => {
    ...
    it("works for min/max and -2/-1/0/1", async () => {
      ...
      expect(negativeTwo.liquidityNet).to.be.eq(
        liquidity0 + liquidity1 + liquidity2
      );
      expect(negativeTwo.liquidityGross).to.be.eq(
        liquidity0 + liquidity1 + liquidity2
      );

      expect(negativeOne.tick).to.be.eq(minus);
      expect(negativeOne.liquidityNet).to.be.eq(
        liquidity3 + liquidity4 - liquidity0
      );
      expect(negativeOne.liquidityGross).to.be.eq(
        liquidity3 + liquidity4 + liquidity0
      );

      expect(zero.tick).to.be.eq(0);
      expect(zero.liquidityNet).to.be.eq(liquidity5 - liquidity1 - liquidity3);
      expect(zero.liquidityGross).to.be.eq(
        liquidity5 + liquidity1 + liquidity3
      );
