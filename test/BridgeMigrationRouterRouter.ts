...
describe("BridgeMigrationRouter", async function() {
    ...
    for(let tokenSymbol of Object.keys(fixture.Migrators)) {
        ...
        it(`Can compute accurately chargeback from AVAX-${tokenSymbol}`, async function() {
            ...
            await migrationRouter.migrateLiquidity(pairAddress, toPairAddress, account.address, liquidityAmount, getDeadline())
            let migratedPair = await getPairContract(toPairAddress);
            let expectedChargeBack0 = (await token0.balanceOf(account.address)).sub(previousBalance0)
            let expectedChargeBack1 = (await token1.balanceOf(account.address)).sub(previousBalance1)
            expect(chargeBack0).to.be.equal(expectedChargeBack0)
            expect(chargeBack1).to.be.equal(expectedChargeBack1)
            expect(await migratedPair.balanceOf(account.address)).to.equal(liquidityAmount);
        })
    }
