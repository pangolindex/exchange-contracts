  ...
  describe("#nextInitializedTickWithinOneWord", () => {
    ...
    describe("lte = false", async () => {
      it("returns tick to right if at initialized tick", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(78, false);
        expect(next).to.eq(84);
        expect(initialized).to.eq(true);
      });
      it("returns tick to right if at initialized tick", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(-55, false);
        expect(next).to.eq(-4);
        expect(initialized).to.eq(true);
      });
      ...
      it("returns the tick directly to the right", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(77, false);
        expect(next).to.eq(78);
        expect(initialized).to.eq(true);
      });
      it("returns the tick directly to the right", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(-56, false);
      ...
      it("returns the next words initialized tick if on the right boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(255, false);
        expect(next).to.eq(511);
        expect(initialized).to.eq(false);
      });
      it("returns the next words initialized tick if on the right boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(-257, false);
      ...
      it("returns the next initialized tick from the next word", async () => {
        ...
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(328, false);
        expect(next).to.eq(340);
        expect(initialized).to.eq(true);
      });
      it("does not exceed boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(508, false);
        expect(next).to.eq(511);
        expect(initialized).to.eq(false);
      });
      ...
      it("skips half word", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(383, false);
        expect(next).to.eq(511);
        expect(initialized).to.eq(false);
      });
    ...
    describe("lte = true", () => {
      ...
      it("will not exceed the word boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(258, true);
      ...
      it("at the word boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(256, true);
      ...
      it("word boundary", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(-257, true);
      ...
      it("entire empty word", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(1023, true);
      ...
      it("halfway through empty word", async () => {
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(900, true);
      ...
      it("boundary is initialized", async () => {
        ...
        const { next, initialized } =
          await tickBitmap.nextInitializedTickWithinOneWord(456, true);
