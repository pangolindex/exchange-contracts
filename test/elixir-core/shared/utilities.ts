...
export function getCreate2AddressWithInitCodeHash(
  factoryAddress: string,
  [tokenA, tokenB]: [string, string],
  fee: number,
  initCodeHash: string
...
export function createPoolFunctions({
  ...
  swapTarget: TestElixirCallee;
  token0: TestERC20;
  token1: TestERC20;
  pool: MockTimeElixirPool;
}): PoolFunctions {
...
export function createMultiPoolFunctions({
  ...
  swapTarget: TestElixirRouter;
  poolInput: MockTimeElixirPool;
  poolOutput: MockTimeElixirPool;
}): MultiPoolFunctions {
