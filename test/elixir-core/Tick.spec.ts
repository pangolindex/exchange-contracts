...
describe("Tick", () => {
  ...
  describe("#tickSpacingToMaxLiquidityPerTick", () => {
    ...
    it("returns the correct value for high fee", async () => {
      ...
      expect(maxLiquidityPerTick).to.eq(
        getMaxLiquidityPerTick(TICK_SPACINGS[FeeAmount.HIGH])
      );
    });
  ...
  describe("#getFeeGrowthInside", () => {
    ...
    it("subtracts upper tick if below", async () => {
      await tickTest.setTick(2, {
        ...
        secondsOutside: 0,
        initialized: true,
        rewardPerLiquidityOutsideX64: 0,
      });
      const { feeGrowthInside0X128, feeGrowthInside1X128 } =
        await tickTest.getFeeGrowthInside(-2, 2, 0, 15, 15);
  ...
  describe("#update", async () => {
    it("flips from zero to nonzero", async () => {
      expect(
        await tickTest.callStatic.update(0, 0, 1, 0, 0, 0, 0, 0, false, 3, 0)
      ).to.eq(true);
    });
    it("does not flip from nonzero to greater nonzero", async () => {
      await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, false, 3, 0);
      expect(
        await tickTest.callStatic.update(0, 0, 1, 0, 0, 0, 0, 0, false, 3, 0)
      ).to.eq(false);
    });
    ...
    it("does not flip from nonzero to lesser nonzero", async () => {
      await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 3, 0);
      expect(
        await tickTest.callStatic.update(0, 0, -1, 0, 0, 0, 0, 0, false, 3, 0)
      ).to.eq(false);
    });
    ...
    it("reverts if total liquidity gross is greater than max", async () => {
      await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 3, 0);
      await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, true, 3, 0);
      await expect(
        tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, false, 3, 0)
      ).to.be.revertedWith("LO");
    });
    it("nets the liquidity based on upper flag", async () => {
      await tickTest.update(0, 0, 2, 0, 0, 0, 0, 0, false, 10, 0);
      await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, true, 10, 0);
      await tickTest.update(0, 0, 3, 0, 0, 0, 0, 0, true, 10, 0);
      await tickTest.update(0, 0, 1, 0, 0, 0, 0, 0, false, 10, 0);
    ...
    it("assumes all growth happens below ticks lte current tick", async () => {
      await tickTest.update(1, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128, 0);
      const {
    ...
    it("does not set any growth fields if tick is already initialized", async () => {
      await tickTest.update(1, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128, 0);
      await tickTest.update(1, 1, 1, 6, 7, 8, 9, 10, false, MaxUint128, 0);
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
        secondsOutside,
    ...
    it("does not set any growth fields for ticks gt current tick", async () => {
      await tickTest.update(2, 1, 1, 1, 2, 3, 4, 5, false, MaxUint128, 0);
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
        secondsOutside,
  ...
  describe("#cross", () => {
    it("flips the growth variables", async () => {
      ...
      await tickTest.cross(2, 7, 9, 8, 15, 10, 0);
      const {
        feeGrowthOutside0X128,
        feeGrowthOutside1X128,
        secondsOutside,
