...
describe("PangolinV2Pool gas tests", () => {
  ...
  for (const feeProtocol of [0, 6]) {
    describe(feeProtocol > 0 ? "fee is on" : "fee is off", () => {
      ...
      beforeEach("load the fixture", async () => {
        ({ swapExact0For1, pool, mint, swapToHigherPrice, swapToLowerPrice } =
        ...
        for (const { description, tickLower, tickUpper } of [
          ...
          describe(description, () => {
            ...
            it("burn when only position using ticks", async () => {
              await snapshotGasCost(
                pool.burn(tickLower, tickUpper, expandTo18Decimals(1))
              );
            });
            it("partial position burn", async () => {
              await snapshotGasCost(
                pool.burn(tickLower, tickUpper, expandTo18Decimals(1).div(2))
              );
            });
