...
describe("NoDelegateCall", () => {
  ...
  const noDelegateCallFixture = async () => {
    const noDelegateCallTestFactory = await ethers.getContractFactory(
      "NoDelegateCallTest"
    );
    const noDelegateCallTest =
      (await noDelegateCallTestFactory.deploy()) as NoDelegateCallTest;
  ...
  beforeEach("deploy test contracts", async () => {
    ({ noDelegateCallTest: base, proxy } = await loadFixture(
      noDelegateCallFixture
    ));
  });
