...
import { PangolinV2PoolSwapTest } from "../../typechain/PangolinV2PoolSwapTest";
...
describe("PangolinV2Pool arbitrage tests", () => {
        ...
        describe(`passive liquidity of ${formatTokenAmount(
          ...
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory(
              "PangolinV2PoolSwapTest"
            );
            const tester =
              (await testerFactory.deploy()) as PangolinV2PoolSwapTest;
          ...
          let tester: PangolinV2PoolSwapTest;
          ...
          async function simulateSwap(
            ...
            const { amount0Delta, amount1Delta, nextSqrtRatio } =
              await tester.callStatic.getSwapResult(
                ...
                amountSpecified,
                sqrtPriceLimitX96 ??
                  (zeroForOne ? MIN_SQRT_RATIO.add(1) : MAX_SQRT_RATIO.sub(1))
              );
          ...
          for (const {
            ...
            describe(description, () => {
              ...
              it("not sandwiched", async () => {
                const { executionPrice, amount1Delta, amount0Delta } =
              ...
              it("sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity", async () => {
                ...
                const { executionPrice: executionPriceAfterFrontrun } =
                  await simulateSwap(zeroForOne, inputAmount);
                zeroForOne
                  ? await swapExact0For1(inputAmount, wallet.address)
                  : await swapExact1For0(inputAmount, wallet.address);
                ...
                const { amount0: amount0Burn, amount1: amount1Burn } =
