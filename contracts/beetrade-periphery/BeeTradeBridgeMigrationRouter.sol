...
contract BeeTradeBridgeMigrationRouter {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'BeeTradeBridgeMigrationRouter: EXPIRED');
        _;
    }

    // makes sure the admin is the one calling protected methods
    modifier onlyAdmin() {
        require(adminRole.has(msg.sender), 'BeeTradeBridgeMigrationRouter: Unauthorized');
        _;
    }
    ...
    function addAdmin(address account) external onlyAdmin {
        require(account != address(0), "BeeTradeBridgeMigrationRouter: Address 0 not allowed");
    ...
    function removeAdmin(address account) external onlyAdmin {
        require(msg.sender != account, "BeeTradeBridgeMigrationRouter: You can't demote yourself");
    ...
    function addMigrator(address tokenAddress, address migratorAddress) external onlyAdmin {
        require(tokenAddress != address(0), "BeeTradeBridgeMigrationRouter: tokenAddress 0 not supported");
        require(migratorAddress != address(0), "BeeTradeBridgeMigrationRouter: migratorAddress 0 not supported");
    ...
    function _allowToken(address tokenAddress, address spenderAddress) internal {
        IBeeTradeERC20(tokenAddress).approve(spenderAddress, type(uint).max);
    }
    ...
    function _addLiquidity(
        ...
        (uint112 reserve0, uint112 reserve1,) = IBeeTradePair(pairToken).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = BeeTradeLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
        ...
        liquidityAmount = IBeeTradePair(pairToken).mint(to);
    }
    ...
    function _rescueLiquidity(
        ...
        (amountTokenA, amountTokenB) = IBeeTradePair(liquidityPair).burn(address(this));
    }
    ...
    function _arePairsCompatible(address pairA, address pairB) internal view {
        ...
        require(
            ...
            IBeeTradePair(pairA).token0() == IBeeTradePair(pairB).token1() ||
            IBeeTradePair(pairA).token1() == IBeeTradePair(pairB).token0() ||
            IBeeTradePair(pairA).token1() == IBeeTradePair(pairB).token1(),
            "BeeTradeBridgeMigrationRouter: Pair does not have one token matching"
        );
    }
    ...
    function _migrateToken(
        ...
        require(
            ...
            "BeeTradeBridgeMigrationRouter: Didn't yield the correct amount"
        );
    }
    ...
    function migrateToken(
        ...
        require(
            ...
            "BeeTradeBridgeMigrationRouter: migrator not registered"
        );
    ...
    function migrateLiquidityWithPermit(
        ...
        IBeeTradePair(liquidityPairFrom).permit(msg.sender, address(this), amount, deadline, v, r, s);
        _migrateLiquidity(
            liquidityPairFrom,
            liquidityPairTo,
            to,
    ...
    function _migrateLiquidity(
        ...
        address tokenToMigrate = IBeeTradePair(liquidityPairFrom).token0();
        if (
            IBeeTradePair(liquidityPairFrom).token0() == IBeeTradePair(liquidityPairTo).token0() ||
            IBeeTradePair(liquidityPairFrom).token0() == IBeeTradePair(liquidityPairTo).token1()
        ) {
            tokenToMigrate = IBeeTradePair(liquidityPairFrom).token1();
        }
        address newTokenAddress = bridgeMigrator[tokenToMigrate];
        require(
            newTokenAddress != address(0),
            "BeeTradeBridgeMigrationRouter: Migrator not registered for the pair"
        );
        require(
            newTokenAddress == IBeeTradePair(liquidityPairTo).token0() ||
            newTokenAddress == IBeeTradePair(liquidityPairTo).token1(),
            "BeeTradeBridgeMigrationRouter: Pair doesn't match the migration token"
        );

        (uint amountTokenA, uint amountTokenB) = _rescueLiquidity(liquidityPairFrom, amount);
        {
            uint amountToSwap = amountTokenA;
            if (tokenToMigrate != IBeeTradePair(liquidityPairFrom).token0()) {
        ...
        (uint changeAmount0, uint changeAmount1, ) = _addLiquidity(
            liquidityPairTo,
            IBeeTradePair(liquidityPairTo).token0(), IBeeTradePair(liquidityPairTo).token1(),
            amountTokenA, amountTokenB, to
        );
        if (changeAmount0 > 0) {
            TransferHelper.safeTransfer(IBeeTradePair(liquidityPairTo).token0(), to, changeAmount0);
        }
        if (changeAmount1 > 0) {
            TransferHelper.safeTransfer(IBeeTradePair(liquidityPairTo).token1(), to, changeAmount1);
        }
    }
    ...
    function _calculateRescueLiquidity(address pairAddress, uint amount) internal view returns (uint amount0, uint amount1) {
        (uint112 reserves0, uint112 reserves1, ) = IBeeTradePair(pairAddress).getReserves();
        uint totalSupply = IBeeTradePair(pairAddress).totalSupply();
    ...
    function calculateChargeBack(
        ...
        (uint112 reserve0, uint112 reserve1,) = IBeeTradePair(liquidityPairTo).getReserves();
        uint quote0 = amountIn0;
        uint quote1 = BeeTradeLibrary.quote(amountIn0, reserve0, reserve1);
        if (quote1 > amountIn1) {
            quote1 = amountIn1;
