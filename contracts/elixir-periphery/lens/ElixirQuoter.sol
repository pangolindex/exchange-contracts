...
contract ElixirQuoter is IElixirQuoter, IElixirSwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IElixirPool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IElixirPool) {
        return IElixirPool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc IElixirSwapCallback
    function elixirSwapCallback(
        ...
        IElixirPool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IElixirPool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
