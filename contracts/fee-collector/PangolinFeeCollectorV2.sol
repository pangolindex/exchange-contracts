...
contract PangolinFeeCollectorV2 is Ownable {

    using SafeERC20 for IERC20;
    using Address for address;

    address public constant FACTORY = 0xefa94DE7a4656D787667C749f7E1223D71E9FD88;
    address public constant ROUTER = 0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106;
    address public constant WAVAX = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    address public immutable GOVERNOR;

    uint256 public constant FEE_DENOMINATOR = 10000;
    uint256 public constant MAX_HARVEST_INCENTIVE = 200;
    ...
    bool public harvestEnabledFlag = true;
    ...
    /// @notice Enable or disable the harvest function for non-owners
    /// @param flag - True to allow and False to disallow
    function setHarvestEnabledFlag(bool flag) external onlyOwner {
        harvestEnabledFlag = flag;
    }
    ...
    function _pullLiquidity(address pair, uint256 balance) internal returns (uint256 amount0, uint256 amount1) {
    ...
    /// @dev Swaps are executed via pairs
    function _swap(address token, address outputToken, uint256 amount) internal {
        address[] memory path;

        if (token == WAVAX || outputToken == WAVAX) {
            path = new address[](2);
            path[0] = token;
            path[1] = outputToken;
        } else {
    ...
    function _collectFees(IPangolinPair[] memory liquidityPairs, address outputToken) internal {
    ...
    function harvest(IPangolinPair[] memory liquidityPairs, bool claimMiniChef) external {
        if (msg.sender != owner()) {
            require(harvestEnabledFlag, "Harvest disabled");
        ...
        if (liquidityPairs.length > 0) {
            require(liquidityPairs.length < 51, "50 pairs max");
