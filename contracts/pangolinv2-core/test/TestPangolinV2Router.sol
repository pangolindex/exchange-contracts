...
abstract contract TestPangolinV2Router is IPangolinV2SwapCallback {
    ...
    function swapForExact0Multi(
        ...
        pools[0] = poolInput;
        IPangolinV2Pool(poolOutput).swap(
            recipient,
            false,
            -amount0Out.toInt256(),
    ...
    function PangolinV2SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IPangolinV2Pool(msg.sender).token0()
                : IPangolinV2Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0
                ? amount0Delta
                : amount1Delta;

            bool zeroForOne = tokenToBePaid ==
                IPangolinV2Pool(pools[0]).token1();
            IPangolinV2Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
                zeroForOne
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IPangolinV2Pool(msg.sender).token0())
                    .transferFrom(payer, msg.sender, uint256(amount0Delta));
            } else {
                IERC20Minimal(IPangolinV2Pool(msg.sender).token1())
