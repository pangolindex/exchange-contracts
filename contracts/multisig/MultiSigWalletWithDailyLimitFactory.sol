...
contract MultiSigWallet {
    ...
    /// @param transactionId Transaction ID.
    /// @return count transactionId Number of confirmations.
    ...
    {
        uint transactionCountTemp = transactionCount;
        uint[] memory transactionIdsTemp = new uint[](transactionCountTemp);
        uint count = 0;
        uint i;
        for (i=0; i<transactionCountTemp; ++i)
            if (   pending && !transactions[i].executed
                || executed && transactions[i].executed)
            {
